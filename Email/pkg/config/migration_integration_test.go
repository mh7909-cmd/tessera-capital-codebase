...
func TestMigration_Integration_LegacyConfigWithoutWorkspace(t *testing.T) {
	...
	if !cfg.Channels.Pico.Enabled {
		t.Error("Pico.Enabled should be true")
	}
	if cfg.Channels.Pico.Token.String() != "test-token" {
		t.Errorf("Pico.Token = %q, want %q", cfg.Channels.Pico.Token.String(), "test-token")
	}
...
`
	...
	// Pico token should be preserved
	if cfg.Channels.Pico.Token.String() != "existing-pico-token-from-env" {
		t.Errorf("Pico token was overwritten: got %q, want %q",
			cfg.Channels.Pico.Token.String(), "existing-pico-token-from-env")
	}
	...
	if reloadedSec.Channels.Pico.Token.String() != "existing-pico-token-from-env" {
...
// TestMigrateConfigV1_Combined verifies that configV1.Migrate applies migrations.
