...
func TestToChannelHashes(t *testing.T) {
	...
	cfg2.Channels.Pico.Enabled = true
	...
	assert.EqualValues(t, []string{"pico"}, added)
	assert.EqualValues(t, []string(nil), removed)

	cfg3 := config.DefaultConfig()
	cfg3.Channels.PicoClient.Enabled = true
	...
	assert.EqualValues(t, []string{"pico"}, removed)
	assert.EqualValues(t, []string{"pico_client"}, added)

	cfg3.Channels.PicoClient.Token.Set("114314")
	...
	assert.EqualValues(t, []string{"pico_client"}, removed)
	assert.EqualValues(t, []string{"pico_client"}, added)

	cc, err := toChannelConfig(cfg3, added)
	assert.NoError(t, err)
	logger.Debugf("cc: %#v", cc.PicoClient)
	assert.Equal(t, "114314", cc.PicoClient.Token.String())
	assert.Equal(t, true, cc.PicoClient.Enabled)

	cc, err = toChannelConfig(cfg2, []string{"pico_client"})
	assert.NoError(t, err)
	logger.Debugf("cc: %#v", cc.PicoClient)
	assert.Equal(t, "", cc.PicoClient.Token.String())
	assert.Equal(t, false, cc.PicoClient.Enabled)
}
