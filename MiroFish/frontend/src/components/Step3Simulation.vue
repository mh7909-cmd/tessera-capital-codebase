  ...
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        <!-- Global Narrative Grid Progress -->
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: twitterCompleted }">
          <div class="platform-header">
            <svg class="platform-icon" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2">
              <circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path>
            </svg>
            <span class="platform-name">Global Narrative Grid</span>
            <span v-if="twitterCompleted" class="status-badge completed">
              <svg class="check-icon" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="3">
          ...
          <div class="platform-stats">
            <span class="stat">
              ...
              <span class="stat-value mono">{{ twitterCurrentRound }}<span class="stat-total">/{{ runStatus.total_rounds || maxRounds || '-' }}</span></span>
            </span>
            <span class="stat">
            ...
            <span class="stat">
              ...
              <span class="stat-value mono">{{ twitterActionsCount }}</span>
            </span>
          </div>
          <!-- Action Matrix -->
          <div class="actions-tooltip">
            <div class="tooltip-title">Signal Spectrum</div>
        ...
        <!-- Community Intel Swarm Progress -->
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: redditCompleted }">
          <div class="platform-header">
            <svg class="platform-icon" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"></path>
            </svg>
            <span class="platform-name">Community Intel Swarm</span>
            <span v-if="redditCompleted" class="status-badge completed">
              <svg class="check-icon" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="3">
                <polyline points="20 6 9 17 4 12"></polyline>
              </svg>
              <span class="badge-text">COMPLETED</span>
          ...
          <div class="platform-stats">
            <span class="stat">
              <span class="stat-label">ROUND</span>
              <span class="stat-value mono">{{ redditCurrentRound }}<span class="stat-total">/{{ runStatus.total_rounds || maxRounds || '-' }}</span></span>
            </span>
            <span class="stat">
              <span class="stat-label">ELAPSED_TIME</span>
            ...
            <span class="stat">
              <span class="stat-label">ACTS</span>
              <span class="stat-value mono">{{ redditActionsCount }}</span>
            </span>
          </div>
          <!-- Action Matrix -->
          <div class="actions-tooltip">
            <div class="tooltip-title">Intelligence Vectors</div>
            <div class="tooltip-actions">
              <span class="tooltip-action">POST</span>
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'SYTHESIZING...' : 'GENERATE STRATEGIC AUDIT' }} 
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div class="timeline-header" v-if="allActions.length > 0">
        <div class="timeline-stats">
          <span class="total-count">TELEMETRY_EVENTS: <span class="mono">{{ allActions.length }}</span></span>
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'SEARCH_POSTS'">
                  <div class="search-info">
                    ...
                    <span class="search-label">Intel Search:</span>
    ...
    <div class="system-logs">
      <div class="log-header">
        <span class="log-title">AUDIT_LOG</span>
        <span class="log-id">{{ simulationId || 'OFFLINE' }}</span>
      </div>
...
const twitterActionsCount = computed(() => {
  // Self-heal: Use the count from allActions if runStatus reports 0
  const feedCount = allActions.value.filter(a => a.platform === 'twitter').length
  return Math.max(feedCount, runStatus.value.twitter_actions_count || 0)
})

const redditActionsCount = computed(() => {
  // Self-heal: Use the count from allActions if runStatus reports 0
  const feedCount = allActions.value.filter(a => a.platform === 'reddit').length
  return Math.max(feedCount, runStatus.value.reddit_actions_count || 0)
})

const twitterCurrentRound = computed(() => {
  // Self-heal: Extract max round from the actual feed
  const roundsFromFeed = allActions.value
    .filter(a => a.platform === 'twitter')
    .map(a => a.round_num || 0)
  const maxFromFeed = roundsFromFeed.length > 0 ? Math.max(...roundsFromFeed) : 0
  return Math.max(maxFromFeed, runStatus.value.twitter_current_round || 0)
})

const redditCurrentRound = computed(() => {
  // Self-heal: Extract max round from the actual feed
  const roundsFromFeed = allActions.value
    .filter(a => a.platform === 'reddit')
    .map(a => a.round_num || 0)
  const maxFromFeed = roundsFromFeed.length > 0 ? Math.max(...roundsFromFeed) : 0
  return Math.max(maxFromFeed, runStatus.value.reddit_current_round || 0)
})

const twitterCompleted = computed(() => {
  // Wait for both the backend to signal completion AND the UI queue to be empty
  const backendDone = runStatus.value.twitter_completed || (twitterCurrentRound.value >= (runStatus.value.total_rounds || props.maxRounds || 10))
  const queueEmpty = telemetryQueue.value.filter(a => a.platform === 'twitter').length === 0
  return backendDone && queueEmpty && !isProcessingQueue.value
})

const redditCompleted = computed(() => {
  // Wait for both the backend to signal completion AND the UI queue to be empty
  const backendDone = runStatus.value.reddit_completed || (redditCurrentRound.value >= (runStatus.value.total_rounds || props.maxRounds || 10))
  const queueEmpty = telemetryQueue.value.filter(a => a.platform === 'reddit').length === 0
  return backendDone && queueEmpty && !isProcessingQueue.value
})

// Log rounds on lock - watch the self-healed values
watch(twitterCurrentRound, (newRound) => {
  if (newRound > prevTwitterRound.value) {
    addLog(`[Grid] R${newRound}/${runStatus.value.total_rounds || props.maxRounds || 10} | T:${formatElapsedTime(newRound)} | A:${twitterActionsCount.value}`)
    prevTwitterRound.value = newRound
  }
})

watch(redditCurrentRound, (newRound) => {
  if (newRound > prevRedditRound.value) {
    addLog(`[Swarm] R${newRound}/${runStatus.value.total_rounds || props.maxRounds || 10} | T:${formatElapsedTime(newRound)} | A:${redditActionsCount.value}`)
    prevRedditRound.value = newRound
  }
})
...
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(twitterCurrentRound.value)
})

const redditElapsedTime = computed(() => {
  return formatElapsedTime(redditCurrentRound.value)
})
...
const doStartSimulation = async () => {
  ...
  try {
    ...
    if (statusRes.success && statusRes.data) {
      ...
      if (runnerStatus === 'running') {
        // Simulation already in progress — attach to live telemetry, do NOT reset
        addLog('✓ Distributed simulation swarm detected. Synchronizing telemetry...')
        phase.value = 1
      ...
      if (runnerStatus === 'completed' || runnerStatus === 'stopped') {
        // Simulation finished before we mounted — load history once, mark done, stop
        addLog('✓ Simulation sequence complete. Loading archived telemetry...')
        phase.value = 2
        runStatus.value = statusRes.data
        emit('update-status', 'completed')
        // Single fetch to populate the action feed; no polling needed
        await fetchRunStatusDetail()
        return
      }
    }
  } catch (e) {
    console.warn('Check run status failed, proceeding to start sequence:', e)
  }

  // ONLY reset if we are actually starting a fresh run
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('Initializing dual-platform parallel simulation...')
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: false, 
      enable_graph_memory_update: !!props.projectData?.graph_id 
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`Maximum simulation rounds configured: ${props.maxRounds}`)
    }
    
    if (params.enable_graph_memory_update) {
      addLog('Dynamic graph memory update enabled')
    } else {
      addLog('Lightweight simulation mode enabled (Graph Sync Off)')
    }
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      addLog('✓ Simulation engine initialized successfully.')
      ...
      const errorMsg = res.error || 'Initialization failed'
    ...
    if (errText.includes('400') && err.message) {
       addLog('✓ Synchronizing with live telemetry feed...')
       phase.value = 1
       startStatusPolling()
       startDetailPolling()
    } else {
      startError.value = err.message
      addLog(`✗ Runtime exception during initialization: ${err.message}`)
      emit('update-status', 'error')
    }
  } finally {
    isStarting.value = false
...
const handleStopSimulation = async () => {
  ...
  addLog('Terminating simulation sequence...')
  
  try {
    const res = await stopSimulation({ simulation_id: props.simulationId })
    
    if (res.success) {
      addLog('✓ Simulation sequence terminated.')
      phase.value = 2
      stopPolling()
      emit('update-status', 'completed')
    } else {
      addLog(`Termination failed: ${res.error || 'Unknown error'}`)
    }
  } catch (err) {
    addLog(`Termination exception: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
const startStatusPolling = () => {
  statusTimer = setInterval(fetchRunStatus, 1000)
}

const startDetailPolling = () => {
  detailTimer = setInterval(fetchRunStatusDetail, 1000)
}
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ All platform simulations synchronized to completion.')
        }
...
const telemetryQueue = ref([])
const isProcessingQueue = ref(false)

const processTelemetryQueue = () => {
  if (telemetryQueue.value.length === 0) {
  ...
  // Slower batch processing for theatricality
  const batchSize = 1
  const batch = telemetryQueue.value.splice(0, batchSize)
  ...
  // High-fidelity pacing: 3-5 seconds between actions for readability
  // If queue is backed up, slightly faster (1.5s), else slow (4s)
  const delay = telemetryQueue.value.length > 5 ? 1500 : 4000
  setTimeout(processTelemetryQueue, delay)
}
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newActionsAdded > 0 && !isProcessingQueue.value) {
        processTelemetryQueue()
      }
    }
  } catch (err) {
    console.warn('Failed to fetch telemetry detail:', err)
  }
}
...
import { formatToTheatrical } from '../utils/clock'

const formatActionTime = (timestamp) => {
  if (!timestamp) return ''
  try {
    return formatToTheatrical(timestamp)
  } catch {
    return ''
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Report synthesis request dispatched. Please standby...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('Initializing report synthesis pipeline...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Synthesis task started: ${reportId}`)
      
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ Synthesis initialization failed: ${res.error || 'Unknown Error'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ Synthesis exception: ${err.message}`)
    isGeneratingReport.value = false
  }
}

// Watch phase to auto-trigger report (Step 3 -> Step 4) - DISABLED as per user request to stay on MiroFish
/*
watch(phase, (newPhase) => {
  if (newPhase === 2) {
    addLog('🚀 [AUTO] Simulation complete. Initiating report generation sequence...')
    setTimeout(() => {
      handleNextStep()
    }, 2000)
  }
...
onMounted(() => {
  addLog('Simulation Monitoring Initialized')
  if (props.simulationId) {
    // Add 5s delay to allow backend autonomous_sim_service to claim the process first
    // This prevents the race condition where both try to start simultaneously
    addLog('Awaiting backend synchronization (5s delay)...')
    setTimeout(() => {
      doStartSimulation()
    }, 5000)
  }
