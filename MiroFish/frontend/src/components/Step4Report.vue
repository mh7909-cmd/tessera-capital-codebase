                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'report_complete'">
                    <div class="complete-banner">
                      ...
                      <span>Technical Audit Synthesis Finalized</span>
                    </div>
                  </template>

                  <!-- Beautification / Upload / Generic Success -->
                  <template v-if="['beautification_start', 'upload_start', 'upload_complete', 'upload_failed'].includes(log.action)">
                    <div class="status-message" :class="{ 
                      'planning': log.action.endsWith('_start'),
                      'success': log.action === 'upload_complete' || (log.details?.message && log.details.message.includes('SUCCESS')),
                      'error': log.action === 'upload_failed'
                    }">
                      {{ log.details?.message }}
                    </div>
                  </template>
...
// Watch isComplete to auto-navigate to interaction stage
watch(isComplete, (newVal) => {
  if (newVal) {
    console.log('🚀 MIROFISH AUTO-PILOT: Report finalized. Navigating to Analysis...')
    setTimeout(() => {
      goToInteraction()
    }, 5000)
  }
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Simulation Context: '),
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Key Metrics (${props.result.facts.length})`)
        ]),
        h('button', {
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Signal Propagation (${props.result.relations.length})`)
        ]),
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `Analysis Sub-trees (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Latest corroborated simulation events'),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Core Entities Identified'),
          ...
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) => 
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount} events`)
              ])
            )
          ),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Signal Propagation Chains'),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift-adjusted analysis sub-trees'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'No critical metrics found'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'No core entities identified'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'No propagation signal detected')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Active Knowledge'),
          ...
          ) : h('div', { class: 'empty-state' }, 'No active knowledge found'),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Historical Perspective'),
          ...
          ) : h('div', { class: 'empty-state' }, 'No historical records found'),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Relevant Entities'),
            h('span', { class: 'panel-count' }, `Total: ${props.result.entities.length}`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'No entities identified'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    onMounted(() => {
      const revealAgents = () => {
        if (visibleAgentCount.value < props.result.interviews.length) {
          visibleAgentCount.value++
          setTimeout(revealAgents, 1500) // 1.5s delay between agents
        }
      }
      revealAgents()
    })
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'agent-tabs' }, 
        props.result.interviews.slice(0, visibleAgentCount.value).map((interview, i) => h('button', {
      ...
      visibleAgentCount.value > 0 && props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        // Agent Profile Card
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: 'quicksearch-header' }, [
        ...
        props.result.query && h('div', { class: 'header-query' }, [
          h('span', { class: 'query-label' }, 'Search Query: '),
      ...
      showTabs.value && h('div', { class: 'quicksearch-tabs' }, [
        ...
        hasNodes.value && h('button', {
          ...
          h('span', { class: 'tab-label' }, `Entities (${props.result.nodes.length})`)
        ])
      ]),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          !showTabs.value && h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Primary Search Results'),
            h('span', { class: 'panel-count' }, `Total: ${props.result.facts.length}`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'No relevant results identified'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `Collapse ▲` : `Show All ${props.result.facts.length} ▼`)
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            // HOLLYWOOD DELAY: Wait 60s before stopping polling to catch GDrive upload logs
            setTimeout(() => {
              stopPolling()
            }, 60000)
