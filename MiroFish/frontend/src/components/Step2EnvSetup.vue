    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create new simulation instance and pull world parameter templates.
          </p>
          ...
          <div v-if="simulationId" class="info-card">
            ...
            <div class="info-row">
              ...
              <span class="info-value mono">{{ taskId || 'Async Task Secured' }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">Agent Persona Synthesis</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combining contextual data, the system auto-analyzes entities from the Knowledge Graph, initializes personas, and injects unique behaviors/memories based on Reality Seeds.
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Projected Cohort</span>
            </div>
            <div class="stat-card">
          ...
          <div v-if="profiles.length > 0" class="profiles-preview">
            <div class="preview-header">
              <span class="preview-title">Synthesized Agent Personas</span>
            </div>
            <div class="profiles-list">
              ...
              >
                ...
                <div class="profile-meta">
                  <span class="profile-profession">{{ profile.profession || 'Professional' }}</span>
                </div>
                <p class="profile-bio">{{ profile.bio || 'No profile data' }}</p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">Parallel World Configuration</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently calibrates world time progression, recommendation weights, agent activity windows, post frequency, and event trigger thresholds based on Reality Seeds.
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                ...
                <div class="config-item">
                  <span class="config-item-label">Active Agents/Hr</span>
              ...
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">Platform 1: Info Plaza</span>
                  </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Automatically seeding hot topics and initial events based on narrative direction to calibrate the simulation's ground state.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        <div class="card-header">
          ...
          <div class="step-status">
            <span v-if="phase >= 4" class="badge processing">ACTIVE</span>
            <span v-else class="badge pending">WAITING</span>
          </div>
        </div>
        ...
        <div class="card-content">
          ...
          <p class="description">Simulation environment configured. Ready to initialize reality mapping.</p>
          
          <!-- 模拟轮数配置 - 只有在配置生成完成且轮数计算出来后才显示 -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Round Configuration</span>
                <span class="section-desc">MiroFish auto-calculated <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours of reality mapping. Each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes.</span>
              </div>
              <label class="switch-control">
                <input type="checkbox" v-model="useCustomRounds">
                <span class="switch-track"></span>
                <span class="switch-label">Manual Override</span>
              </label>
            </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>Target Cohort Scope (100 Agents): Estimated Time ~{{ Math.round(customMaxRounds * 0.6) }} Mins</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        For 100 Agents: Estimated ~{{ Math.round(autoGeneratedRounds * 0.6) }} Minutes
                      </span>
                    </div>
                    <div class="auto-desc">
          ...
          <div class="action-group dual">
            ...
            >
              Launch Parallel Simulation Swarm ➝
            </button>
          </div>
        </div>
      </div>
...
const hasTriggeredAutoPilot = ref(false)
...
// --- 自动化触发逻辑 ---
// 监听 phase 状态，当进入阶段 4 (准备完成) 时，自动配置并启动模拟
watch(phase, (newPhase) => {
  if (newPhase === 4 && !hasTriggeredAutoPilot.value) {
    hasTriggeredAutoPilot.value = true
    // 延迟 2 秒后开始执行自动化
    setTimeout(() => {
      if (phase.value === 4) {
        addLog('🚀 MIROFISH: Environment stabilized. Executing autonomous launch sequence...')
        
        // 1. 自动切换至自定义模式
        useCustomRounds.value = true
        // 2. 强制设置为 20 轮 (Elite Logic Target)
        customMaxRounds.value = 20
        addLog('✓ Mode: Custom (Locked at 20 Rounds)')
        
        // 3. 延迟后自动触发开始按钮
        setTimeout(() => {
          if (phase.value === 4) {
            addLog('⚡ INITIATING SIMULATION SWARM...')
...
const startPrepareSimulation = async () => {
  ...
  // 立即标记第一步完成，开始第二步（Agent人设生成）
  phase.value = 1
  addLog(`Simulation context verified: ${props.simulationId}`)
  addLog('Orchestrating autonomous environment parameters...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ 环境搭建完成，正在自动启动模拟...')
        emit('update-status', 'completed')
        
        // AUTO-PILOT: 自动进入下一步（启动模拟）
        setTimeout(() => {
          if (!hasTriggeredAutoPilot.value) {
            hasTriggeredAutoPilot.value = true
            handleStartSimulation()
          }
        }, 2000)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('✓ 环境搭建完成，正在自动恢复模拟...')
        phase.value = 4
        emit('update-status', 'completed')
        
        // AUTO-PILOT: 自动回复并启动模拟
        setTimeout(() => {
          if (!hasTriggeredAutoPilot.value) {
            hasTriggeredAutoPilot.value = true
            handleStartSimulation()
