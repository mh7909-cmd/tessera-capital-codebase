"""
Graph API Routes
Using project context mechanism, server-side persistent state.
"""
...
def reset_project(project_id: str):
    """
    Reset project status (for graph rebuilding)
    """
    project = ProjectManager.get_project(project_id)
...
def generate_ontology():
    """
    Step 1: Upload files, analyze and generate ontology definition
    ...
    """
    try:
        logger.info("--- Initiating Institutional Ontology Synthesis ---")
        ...
        if not document_texts:
            ...
            return jsonify({
                "success": False,
                "error": "No documents successfully processed, please check file formats"
            }), 400
...
def build_graph():
    """
    Step 2: Build knowledge graph based on project_id
    ...
    """
    try:
        logger.info("--- Commencing Knowledge Graph Construction ---")
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph construction in progress, do not resubmit. To force rebuild, add force: true",
                "task_id": project.graph_build_task_id
            }), 400
        
        # Reset if force rebuild
        ...
        logger.info(f"Creating graph construction task: task_id={task_id}, project_id={project_id}")
        ...
        def build_task():
            ...
            try:
                ...
                task_manager.update_task(
                    ...
                    message="Initializing graph construction service..."
                )
                ...
                task_manager.update_task(
                    task_id,
                    message="Text chunking in progress...",
                    progress=5
                )
                ...
                task_manager.update_task(
                    task_id,
                    message=f"Starting to add {total_chunks} chunks...",
                    progress=15
                )
                ...
                task_manager.update_task(
                    task_id,
                    message="Waiting for Zep processing...",
                    progress=55
                )
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": "Graph construction task started, check progress via /task/{task_id}"
            }
        })
...
def get_graph_data(graph_id: str):
    ...
    try:
        ...
        logger.error(f"Graph Data Sync Failure: {str(e)}")
