...
INTERVIEW_PROMPT_PREFIX = "Based on your persona, all past memories and actions, please reply with plain text directly without calling any tools: "
...
def prepare_simulation():
    ...
    try:
        ...
        # 获取模拟需求 (支持从请求中覆盖)
        simulation_requirement = data.get('simulation_requirement') or project.simulation_requirement or ""
        ...
        # 获取文档文本 (支持从请求中覆盖)
        document_text = data.get('document_text') or ProjectManager.get_extracted_text(state.project_id) or ""
...
def get_simulation(simulation_id: str):
    ...
    try:
        ...
        # Merge runtime telemetry if simulation is running or completed
        try:
            run_state = SimulationRunner.get_run_state(simulation_id)
            if run_state:
                run_data = run_state.to_dict()
                # Update round and action counts (ensuring field names match SimulationRunState.to_dict)
                result["twitter_current_round"] = run_data.get("twitter_current_round", 0)
                result["reddit_current_round"] = run_data.get("reddit_current_round", 0)
                result["twitter_actions_count"] = run_data.get("twitter_actions_count", 0)
                result["reddit_actions_count"] = run_data.get("reddit_actions_count", 0)
                result["twitter_completed"] = run_data.get("twitter_completed", False)
                result["reddit_completed"] = run_data.get("reddit_completed", False)
                result["total_rounds"] = run_data.get("total_rounds", 0)
                result["runner_status"] = run_data.get("runner_status")
        except Exception as re:
            logger.warning(f"Could not merge run_state: {re}")
...
def start_simulation():
    ...
    try:
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                logger.warning(f"Graph ID missing for simulation {simulation_id}. Disabling graph memory update.")
