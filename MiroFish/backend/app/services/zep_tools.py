...
class SearchResult:
    ...
    def to_text(self) -> str:
        """Converts to text format for LLM comprehension"""
        text_parts = [f"Search Query: {self.query}", f"Found {self.total_count} relevant information results"]
...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        """Converts to detailed text format for LLM comprehension"""
        text_parts = [
            f"## Deep Future Prediction Analysis",
            f"Analysis Query: {self.query}",
            f"Prediction Scenario: {self.simulation_requirement}",
            f"\n### Prediction Data Statistics",
            f"- Related Prediction Facts: {self.total_facts} total",
            f"- Involved Entities: {self.total_entities} total",
            f"- Relationship Chains: {self.total_relationships} total"
        ]
        
        # 子问题
        if self.sub_queries:
            text_parts.append(f"\n### Analyzed Sub-problems")
            for i, sq in enumerate(self.sub_queries, 1):
                text_parts.append(f"{i}. {sq}")
        
        # 语义搜索结果
        if self.semantic_facts:
            text_parts.append(f"\n### [Key Facts] (Please quote these raw original sources in the report)")
        ...
        if self.entity_insights:
            ...
            for entity in self.entity_insights:
                ...
                if entity.get('related_facts'):
                    text_parts.append(f"  Related Facts: {len(entity.get('related_facts', []))} total")
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        """Converts to text format (Full version, no truncation)"""
        text_parts = [
            f"## Panoramic Future Rehearsal View",
            f"Search Query: {self.query}",
            f"\n### Simulation Statistics",
            f"- Total Nodes: {self.total_nodes}",
            f"- Total Relationship Connections: {self.total_edges}",
            f"- Current Active Facts: {self.active_count} total",
            f"- Historical/Expired Facts: {self.historical_count} total"
        ]
        
        # Current active facts (Full output, no truncation)
        if self.active_facts:
            text_parts.append(f"\n### [Latest Active Facts] (Direct simulation observations)")
            for i, fact in enumerate(self.active_facts, 1):
                text_parts.append(f"{i}. \"{fact}\"")
        
        # Historical/Expired facts (Full output, no truncation)
        if self.historical_facts:
            text_parts.append(f"\n### [Historical Rehearsal Records] (Past evolved states)")
...
class InterviewResult:
    ...
    def to_text(self) -> str:
        """Converts to detailed text format for LLM reporting and citation"""
        text_parts = [
            "## In-Depth Stakeholder Interview Report",
            f"**Interview Topic:** {self.interview_topic}",
            f"**Respondents:** {self.interviewed_count} / {self.total_agents} Simulated Agents",
            "\n### Stakeholder Selection Rationale",
            self.selection_reasoning or "(Automated selection)",
            "\n---",
            "\n### Live Interview Logs",
        ]
        ...
        if self.interviews:
            for i, interview in enumerate(self.interviews, 1):
                text_parts.append(f"\n#### Interview Session #{i}: {interview.agent_name}")
                text_parts.append(interview.to_text())
                text_parts.append("\n---")
        else:
            text_parts.append("(No interview records found)\n\n---")

        text_parts.append("\n### Interview Synthesis & Core Perspectives")
...
class ZepToolsService:
    ...
    def _call_with_retry(self, func, operation_name: str, max_retries: int = None):
        """API call with retry mechanism and 404 hardening"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if status_code == 404:
                    logger.warning(f"Zep {operation_name} returned 404 Not Found. Skipping retries.")
    ...
    def search_graph(
        ...
        # 截断查询以符合 Zep 限制 (400字符)
        if len(query) > 400:
            logger.warning(f"查询过长 ({len(query)} 字符)，正在截断为 400 字符...")
            query = query[:400]
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        try:
            ...
            if "status_code: 404" in str(e) or "404" in str(e):
                logger.warning(f"图谱 {graph_id} 未找到 (404)，返回空节点列表")
                return []
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        try:
            ...
            if "status_code: 404" in str(e) or "404" in str(e):
                logger.warning(f"图谱 {graph_id} 未找到 (404)，返回空边列表")
                return []
    ...
    def get_graph_statistics(self, graph_id: str) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            future_nodes = executor.submit(self.get_all_nodes, graph_id)
            future_edges = executor.submit(self.get_all_edges, graph_id)
    ...
    def get_simulation_context(
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            future_search = executor.submit(self.search_graph, graph_id=graph_id, query=simulation_requirement, limit=limit)
            future_stats = executor.submit(self.get_graph_statistics, graph_id)
            future_nodes = executor.submit(self.get_all_nodes, graph_id)
            
            search_result = future_search.result()
            stats = future_stats.result()
            all_nodes = future_nodes.result()
    ...
    def insight_forge(
        ...
        # Step 2: 对每个子问题进行并行语义搜索 (OPTIMIZED)
        from concurrent.futures import ThreadPoolExecutor
        all_facts = []
        all_edges = []
        seen_facts = set()
        
        def run_sub_query_search(sub_query):
        ...
        with ThreadPoolExecutor(max_workers=min(len(sub_queries), 2)) as executor:
            search_results = list(executor.map(run_sub_query_search, sub_queries))
        ...
        # 并行获取所有相关实体的详情 (OPTIMIZED)
        entity_insights = []
        node_map = {}  # 用于后续关系链构建
        uuids_to_fetch = [u for u in list(entity_uuids) if u]
        
        def fetch_entity_insight(uuid):
            try:
        ...
        with ThreadPoolExecutor(max_workers=min(len(uuids_to_fetch), 2) if uuids_to_fetch else 1) as executor:
            results = list(executor.map(fetch_entity_insight, uuids_to_fetch))
            for res in results:
                if res:
                    node_map[res["uuid"]] = res.pop("node_obj")
                    entity_insights.append(res)
    ...
    def _generate_sub_queries(
        ...
        """
        Generates sub-queries using LLM.
        
        Decomposes complex problems into multiple independently retrievable sub-queries.
        """
        system_prompt = """You are a professional problem analysis expert. Your task is to decompose a complex problem into multiple sub-problems that can be independently observed in a simulated world.

Requirements:
1. Each sub-query should be specific enough to locate relevant Agent behaviors or events in the simulated world.
...
Please decompose the following problem into {max_queries} sub-queries:
{query}
...
Return a list of sub-queries in JSON format."""
    ...
    def panorama_search(
        ...
        # Parallel retrieval of all nodes and edges (OPTIMIZED)
        with ThreadPoolExecutor(max_workers=2) as executor:
            future_nodes = executor.submit(self.get_all_nodes, graph_id)
            future_edges = executor.submit(self.get_all_edges, graph_id, include_temporal=True)
            all_nodes = future_nodes.result()
            all_edges = future_edges.result()
    ...
    def interview_agents(
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "4. Answer questions one by one, each starting with 'Question X:' (where X is the number).\n"
            "5. Use double empty lines between answers.\n"
            "6. Provide substantial content, at least 2-3 sentences per question.\n\n"
        )
        ...
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                response_text = f"【Twitter Platform Analysis】\n{twitter_text}\n\n【Reddit Platform Analysis】\n{reddit_text}"
                ...
                clean_text = re.sub(r'Wait for response \d+[：:]\s*', '', clean_text)
            ...
            result.summary = f"Interview failed: {str(e)}. The simulation environment might be closed. Please ensure OASIS is running."
            return result
        except Exception as e:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean tool call JSON wrappers from agent responses"""
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable candidates to interview from a list of simulation agents based on the interview requirements.
...
3. Select a diverse range of perspectives (e.g., support, opposition, neutral, professional, etc.).
...
Please select up to {max_agents} agents most suitable for the interview and provide reasoning."""
    ...
    def _generate_interview_questions(
        ...
        system_prompt = """You are a professional journalist/interviewer. Generate 3-5 deep interview questions based on the interview requirements.

Question Requirements:
1. Open-ended questions to encourage detailed answers.
2. Different agents may have different answers tailored to their roles.
3. Cover multiple dimensions such as facts, views, and feelings.
4. Natural language, like a real interview.
5. Limit each question to 50 words, concise and clear.
...
Please generate 3-5 interview questions."""
        ...
        try:
            # Hardcoded Mandatory Questions for Firefly Aerospace Demo
            return [
                "What are the main risks in Firefly's propulsion architecture?",
                "Can they actually meet their contract obligations with current manufacturing?",
                "Are there any other supply chain or infrastructure bottlenecks?"
            ]
            ...
            return [
                ...
                "How does this matter impact you or the group you represent?",
...
1. Extract main ideas from all parties.
2. Identify consensus and divergence in views.
3. Highlight valuable quotes.
4. Objective and neutral, do not favor any party.
5. Limit to 1000 words.

Format Constraints (MUST BE FOLLOWED):
- Use plain text paragraphs, separated by empty lines.
- Do not use Markdown headers (e.g., #, ##, ###).
- Do not use horizontal lines (e.g., ---, ***).
- Use double quotes " " for direct quotes.
- You can use **bold** for keywords, but no other Markdown syntax."""
