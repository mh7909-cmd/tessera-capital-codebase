...
class ZepGraphMemoryUpdater:
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        ...
        # 2. 同步到 Neo4j (阻塞，符合用户对内存一致性的要求)
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            
            # 生成 Embedding
            texts_to_embed = [a.to_episode_text() for a in activities]
            embeddings = loop.run_until_complete(self.embedding_client.get_embeddings(texts_to_embed))
            
            # 准备 Neo4j 节点数据
            neo_nodes = []
            for i, activity in enumerate(activities):
                node_uuid = f"activity_{activity.platform}_{activity.agent_id}_{int(time.time()*1000)}_{i}"
                neo_nodes.append({
                    "uuid": node_uuid,
                    "name": activity.action_type,
                    "labels": ["AgentActivity", activity.platform.capitalize()],
                    ...
                    "attributes": {
                        ...
                        **activity.action_args
                    },
                    "embedding": embeddings[i] if i < len(embeddings) else None,
                    "created_at": activity.timestamp
                })
            
            # self.neo4j.save_graph_data(self.graph_id, neo_nodes, []) # Disable legacy sync
            pass
            loop.close()
            logger.info(f"已同步 {len(activities)} 条活动到 Neo4j")
        except Exception as e:
            logger.error(f"同步活动到 Neo4j 失败: {e}")
    ...
    def flush(self):
        """
        公开的阻塞刷新方法
        确保队列和缓冲区中的所有活动都已发送并同步到 Neo4j
        用于轮次切换时的内存一致性保证
        """
        logger.info(f"开始阻塞刷新图谱记忆: graph_id={self.graph_id}")
        # 1. 等待队列清空
        while not self._activity_queue.empty():
            time.sleep(0.1)
        ...
        # 2. 强制刷新所有缓冲区
        self._flush_remaining()
        logger.info(f"图谱记忆阻塞刷新完成: graph_id={self.graph_id}")
