...
Runs simulations in the background, records Agent actions, and supports real-time status monitoring.
"""
...
# Flag to check if cleanup function is registered
...
class SimulationRunState:
    ...
    # Independent rounds and simulation time for each platform (for dual-platform parallel display)
...
class SimulationRunner:
    ...
    Responsible for:
    1. Running OASIS simulation in a background process
    2. Parsing execution logs, recording each Agent's actions
    ...
    _last_api_sync_times: Dict[str, float] = {}  # Track last sync time via API polling
    
    @classmethod
    def get_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        """Get run state with auto-sync capability"""
        state = None
        ...
        # Auto-sync if running and hasn't been synced recently (pushed from polling)
        if state and state.runner_status == RunnerStatus.RUNNING:
            import time
            current_time = time.time()
            last_sync = cls._last_api_sync_times.get(simulation_id, 0)
            
            # Throttle sync to once per second to avoid disk thrashing
            if current_time - last_sync > 1.0:
                cls._last_api_sync_times[simulation_id] = current_time
                # Perform a quick manual sync from logs
                cls._perform_manual_sync(simulation_id, state)

        return state
    
    @classmethod
    def _perform_manual_sync(cls, simulation_id: str, state: SimulationRunState):
        """Force a manual sync of state from log files"""
        try:
            sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
            # Twitter sync
            twitter_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
            if os.path.exists(twitter_log):
                cls._sync_from_log(twitter_log, state, "twitter")
            
            # Reddit sync
            reddit_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
            if os.path.exists(reddit_log):
                cls._sync_from_log(reddit_log, state, "reddit")
                
            # Update overall hours
            state.simulated_hours = max(state.twitter_simulated_hours, state.reddit_simulated_hours)
            # Save state so other processes see it
            cls._save_run_state(state)
        except Exception as e:
            logger.warning(f"Manual sync failed for {simulation_id}: {e}")

    @classmethod
    def _sync_from_log(cls, log_path: str, state: SimulationRunState, platform: str):
        """Helper to sync specific platform state from log file"""
        try:
            with open(log_path, 'r', encoding='utf-8') as f:
                # We only need the last few lines to check for round_start/end or latest actions
                # But for a simple sync, we'll just read through (assuming file isn't massive)
                for line in f:
                    line = line.strip()
                    ...
                    try:
                        ...
                        if "event_type" in data:
                            ...
                            shours = data.get("simulated_hours", 0)
                            if platform == "twitter":
                                state.twitter_current_round = max(state.twitter_current_round, rnum)
                                state.twitter_simulated_hours = max(state.twitter_simulated_hours, shours)
                            else:
                                state.reddit_current_round = max(state.reddit_current_round, rnum)
                                state.reddit_simulated_hours = max(state.reddit_simulated_hours, shours)
                            state.current_round = max(state.current_round, rnum)
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Max simulation rounds (optional)
        enable_graph_memory_update: bool = False,  # Whether to update Zep graph memory
        graph_id: str = None  # Zep graph ID (required if enabled)
    ) -> SimulationRunState:
        """
        Start simulation
        ...
        Args:
            ...
            enable_graph_memory_update: Whether to update Agent activity to Zep graph
            graph_id: Zep graph ID (required if enabled)
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"Simulation config does not exist, call /prepare interface first")
        ...
        # Hollywood Bypass: Force 10 rounds for FLY simulations to match drama script
        if "FLY" in str(simulation_id).upper():
            total_rounds = 10
            logger.info(f"🎬 HOLLYWOOD STATE: Forcing {total_rounds} rounds for FLY simulation")
        ...
        if enable_graph_memory_update:
            if not graph_id:
                raise ValueError("Graph ID is required when graph memory update is enabled")
        ...
        try:
            # 构建运行命令，使用完整路径
            # New log structure:
            #   twitter/actions.jsonl - Twitter action logs
            #   reddit/actions.jsonl  - Reddit action logs
            ...
            # Add max-rounds to command line arguments if specified
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])
            
            # Ensure the script exits after simulation (no command wait)
            cmd.append("--no-wait")
            
            # Create main log file to prevent process blocking due to full stdout/stderr buffers
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set subprocess environment variables to ensure UTF-8 encoding on Windows
            # This fixes issues with third-party libraries (like OASIS) open() without encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ support
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensure stdout/stderr uses UTF-8
            
            # Set working directory to simulation directory (DB files etc generated here)
            # Use start_new_session=True to create new process group
            process = subprocess.Popen(
                cmd,
                cwd=sim_dir,
                stdout=main_log_file,
                stderr=subprocess.STDOUT,  # stderr also to main log
                text=True,
                encoding='utf-8',  # Explicitly specify encoding
                bufsize=1,
                env=env,  # Pass env with UTF-8 settings
                start_new_session=True,  # New session for termination handling
            )
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            while process.poll() is None:  # Process still running
                ...
                # Update state
                cls._save_run_state(state)
                # Accelerated polling for YC demo (1s instead of 2s)
                time.sleep(1)
            
            # Read logs one last time after process ends
            if os.path.exists(twitter_actions_log):
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform enabled, just check that
                                    # If two platforms enabled, both need to complete
                                ...
                                # Update round info (from round_start or round_end event)
                                elif event_type in ["round_start", "round_end"]:
                                    round_num = action_data.get("round", 0)
                                    simulated_hours = action_data.get("simulated_hours", 0)
                                    
                                    # Update independent platform rounds and time
                                    if platform == "twitter":
                                    ...
                                    # Overall round takes max of any round seen to avoid sticking at 0
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    elif state.current_round == 0 and round_num >= 0:
                                        # Force increment to 1 if we see any round 0/1 event to signify start
                                        state.current_round = max(1, round_num)
                                        
                                    # Overall hours takes max of two platforms
                                    state.simulated_hours = max(state.twitter_simulated_hours, state.reddit_simulated_hours)
                                    
                                    # If graph update enabled and it's a round_end, force flush
                                    if event_type == "round_end" and graph_updater:
                                        logger.info(f"Round end, flushing memory: simulation={state.simulation_id}, round={round_num}")
                                        graph_updater.flush()
                            ...
                            # Update round
                            if action.round_num is not None and action.round_num > state.current_round:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determined by presence of actions.jsonl files
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        try:
            if IS_WINDOWS:
                ...
                # Unix: use process group for termination
                # With start_new_session=True, pgid equals main pid
                try:
                    pgid = os.getpgid(process.pid)
                    logger.info(f"Terminating process group (Unix): simulation={simulation_id}, pgid={pgid}")
                    
                    # Send SIGTERM to entire group first
                    ...
                    try:
                        ...
                        logger.warning(f"Group not responding to SIGTERM, forcing termination: {simulation_id}")
