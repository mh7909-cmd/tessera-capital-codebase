...
class PlatformActionLogger:
    ...
    def __init__(self, platform: str, base_dir: str):
        ...
        # Hollywood Demo: Virtual Time Offset (Start at 04:53:00)
        now = datetime.now()
        target_start = now.replace(hour=4, minute=53, second=0, microsecond=0)
        self._time_offset = target_start - now
    
    def _get_timestamp(self) -> str:
        """Returns the virtual cinematic timestamp."""
        virtual_now = datetime.now() + self._time_offset
