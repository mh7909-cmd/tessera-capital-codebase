...
class LLMModel(BaseModel):
    ...
    def has_json_mode(self) -> bool:
        ...
        if self.is_deepseek() or self.is_gemini() or self.provider == ModelProvider.NVIDIA:
            return False
...
def get_model(model_name: str, model_provider: ModelProvider, api_keys: dict = None) -> ChatOpenAI | ChatGroq | ChatOllama | GigaChat | None:
    if model_provider == ModelProvider.GROQ:
        ...
        return ChatGroq(model=model_name, api_key=api_key, timeout=60)
        ...
        return ChatOpenAI(model=model_name, api_key=api_key, base_url=base_url, timeout=60)
        ...
        return ChatAnthropic(model=model_name, api_key=api_key, timeout=60)
        ...
        return ChatDeepSeek(model=model_name, api_key=api_key, timeout=60)
        ...
        return ChatGoogleGenerativeAI(model=model_name, api_key=api_key, timeout=60)
        ...
        return ChatOllama(
            model=model_name,
            base_url=base_url,
            timeout=120, # Ollama can be slower locally
        )
        ...
        return ChatXAI(model=model_name, api_key=api_key, timeout=60)
        ...
        api_key = (api_keys or {}).get("NVIDIA_API_KEY") or os.getenv("NVIDIA_API_KEY")
        base_url = os.getenv("NVIDIA_BASE_URL", "https://integrate.api.nvidia.com/v1")
        if not api_key:
            print(f"API Key Error: Please make sure NVIDIA_API_KEY is set in your .env file.")
            raise ValueError("NVIDIA API key not found. Please make sure NVIDIA_API_KEY is set in your .env file.")
        
        # Use ChatOpenAI for OpenAI-compatible NVIDIA API
        # Only enable Thinking Mode for models that specifically require it (e.g., Nemotron-3 Super or Gemma Thinking)
        enable_thinking = any(x in model_name.lower() for x in ["thinking", "nemotron-3-super"])
        ...
        return ChatOpenAI(
            ...
            api_key=api_key,
            base_url=base_url,
            timeout=180 if enable_thinking else 60,
