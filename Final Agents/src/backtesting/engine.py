...
class BacktestEngine:
    ...
    def _record_feedback(
        ...
        """Append per-agent signal vs realized return for FeedbackTracker."""
        signals = agent_output.get("analyst_signals") or {}
        ft = FeedbackTracker()
        for ticker in self._tickers:
            ...
            for agent_id, block in signals.items():
                if "risk_management" in agent_id:
                    continue
                if not isinstance(block, dict) or ticker not in block:
                    continue
                row = block[ticker]
                pred = row.get("signal")
                if pred not in ("bullish", "bearish", "neutral"):
                    continue
                try:
                ...
                ft.record_trade_outcome(
                    agent_name=agent_id,
                    predicted_signal=pred,  # type: ignore[arg-type]
    ...
    def run_backtest(self) -> PerformanceMetrics:
        ...
        for current_date in dates:
            ...
            try:
                ...
                for ticker in self._tickers:
                    try:
                        ...
                        if len(price_data) >= 2:
                            prior_prices[ticker] = float(price_data.iloc[0]["close"])
                        else:
                            prior_prices[ticker] = current_prices[ticker]
                    except Exception:
                        missing_data = True
                        break
                if missing_data:
            ...
            self._record_feedback(agent_output, current_date_str, prior_prices, current_prices)
