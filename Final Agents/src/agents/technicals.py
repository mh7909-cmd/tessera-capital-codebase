...
def technical_analyst_agent(state: AgentState, agent_id: str = "technical_analyst_agent"):
    ...
    for ticker in tickers:
        ...
        conf_pct = round(combined_signal["confidence"] * 100)
        technical_analysis[ticker] = enrich_deterministic_cot(
            agent_key="technical_analyst",
            philosophy="ADAPTIVE INTELLIGENCE: Synthesize price action vs institutional research context.",
            criteria_bullets=[
                f"Trend: {trend_signals['signal']}",
                f"Mean reversion: {mean_reversion_signals['signal']}",
                f"Momentum: {momentum_signals['signal']}",
                f"Volatility: {volatility_signals['signal']}",
                f"Stat arb: {stat_arb_signals['signal']}",
            ],
            signal=combined_signal["signal"],
            confidence=conf_pct,
            conviction_reason=f"Institutional multi-factor ensemble favors {combined_signal['signal']} with {conf_pct}% confidence.",
            invalidation_condition="Regime shift: price action deviates from institutional research and trend breaks.",
            reasoning_legacy=reasoning_detail,
        )
