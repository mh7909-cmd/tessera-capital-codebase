...
from src.agents.cot_schema import enrich_deterministic_cot
...
def valuation_analyst_agent(state: AgentState, agent_id: str = "valuation_analyst_agent"):
    ...
    for ticker in tickers:
        ...
        # --- Simulation First Data Strategy ---
        progress.update_status(agent_id, ticker, "Extracting institutional financial data")
        
        # Priority 1: Extract from Simulation Research Context
        research_context = state["data"].get("research_context", "")
        
        # Hardcoded High-Fidelity Profile for FLY Demo
        if ticker.upper() == "FLY":
            # These values are extracted from the Layer 1 Google Sheet & Layer 3 Transcript
            li_curr_data = {
                "net_income": -133400000.0, # From Q3
                "depreciation_and_amortization": 45000000.0,
                "capital_expenditure": 150000000.0,
                "working_capital": 85000000.0,
                "total_debt": 1020000000.0, # From metrics data
                "cash_and_equivalents": 996000000.0, # From dossier
                "revenue": 30800000.0, # Q3
                "free_cash_flow": -62000000.0 # Q3
            }
            most_recent_metrics_data = {
                "market_cap": 5730000000.0, # Matches Dossier
                "enterprise_value": 5580000000.0, # Matches Dossier
                "enterprise_value_to_ebitda_ratio": -28.04, # Matches Dossier
                "revenue_growth": 1.0,
            ...
            # Create mock objects to satisfy the existing calculation logic
            from dataclasses import make_dataclass
            Metrics = make_dataclass("Metrics", most_recent_metrics_data.keys())
            LineItems = make_dataclass("LineItems", li_curr_data.keys())
            
            most_recent_metrics = Metrics(**most_recent_metrics_data)
            li_curr = LineItems(**li_curr_data)
            li_prev = LineItems(**{k: v*0.9 for k,v in li_curr_data.items()}) # Mock growth
            line_items = [li_curr, li_prev]
            financial_metrics = [most_recent_metrics]
        else:
            # Fallback for non-demo tickers (Still disabled API to prevent errors)
            progress.update_status(agent_id, ticker, "Failed: API Disabled. Use Simulation Data.")
            continue
        ...
        if getattr(li_curr, 'working_capital', None) is not None and getattr(li_prev, 'working_capital', None) is not None:
        ...
        owner_val = calculate_owner_earnings_value(
            ...
            depreciation=getattr(li_curr, 'depreciation_and_amortization', 0),
            capex=getattr(li_curr, 'capital_expenditure', 0),
            working_capital_change=wc_change,
            growth_rate=getattr(most_recent_metrics, 'earnings_growth', 0.05) or 0.05,
        )
        ...
        dcf_results = calculate_dcf_scenarios(
            ...
            growth_metrics={
                'revenue_growth': getattr(most_recent_metrics, 'revenue_growth', 0),
                'fcf_growth': getattr(most_recent_metrics, 'free_cash_flow_growth', 0),
                'earnings_growth': getattr(most_recent_metrics, 'earnings_growth', 0)
            },
            wacc=wacc,
            market_cap=most_recent_metrics.market_cap or 0,
        ...
        rim_val = calculate_residual_income_value(
            ...
            book_value_growth=getattr(most_recent_metrics, 'book_value_growth', 0.03) or 0.03,
        )

        # ------------------------------------------------------------------
        # Aggregate & signal
        # ------------------------------------------------------------------
        market_cap = getattr(most_recent_metrics, "market_cap", 0)
        if not market_cap:
        ...
        # Add research context and DCF scenario summary if available
        reasoning["research_context"] = state["data"].get("research_context", "No external documents provided.")
        ...
        valuation_analysis[ticker] = enrich_deterministic_cot(
            agent_key="valuation_analyst",
            philosophy="ADAPTIVE INTELLIGENCE: Triangulate fair value by synthesizing multi-model DCF scenarios vs institutional research.",
            criteria_bullets=[f"{k}: {v.get('signal', 'n/a') if isinstance(v, dict) else 'info'}" for k, v in list(reasoning.items())[:5]],
            signal=signal,
            confidence=confidence,
            conviction_reason=f"Institutional blended valuation models imply {signal} conviction vs market cap.",
            invalidation_condition="WACC or growth assumptions prove materially wrong vs realized cash flows and research context.",
            reasoning_legacy=reasoning,
        )
...
def calculate_ev_ebitda_value(financial_metrics: list):
    ...
    if not (ev and ev_ebitda):
        return 0
    if ev_ebitda == 0:
        return 0

    ebitda_now = ev / ev_ebitda
    ratios = [getattr(m, 'enterprise_value_to_ebitda_ratio', None) for m in financial_metrics]
    ...
    net_debt = (getattr(m0, 'enterprise_value', 0) or 0) - (getattr(m0, 'market_cap', 0) or 0)
