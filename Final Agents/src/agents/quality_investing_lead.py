...
def quality_investing_lead_agent(state: AgentState, agent_id: str = "quality_investing_lead"):
    """
    Analyzes stocks using quality investing principles and mental models.
    ...
    analysis_data = {}
    quality_analysis_payload = {}
    
    for ticker in tickers:
        progress.update_status(agent_id, ticker, "Fetching financial metrics")
        metrics = get_financial_metrics(ticker, end_date, period="annual", limit=10, api_key=api_key)  # Professional analysis looks at longer periods
        ...
        financial_line_items = search_line_items(
            ...
            end_date,
            period="annual",
            limit=10,  # Strategy examines long-term trends
            api_key=api_key,
        )
        ...
        # Strategy values management with skin in the game
        ...
        # Avoid businesses with frequent negative press
        ...
        progress.update_status(agent_id, ticker, "Calculating quality-focused valuation")
        valuation_analysis = calculate_quality_valuation(financial_line_items, market_cap)
 
        # Combine partial scores with weighting preferences
        # Weight quality and predictability higher than current valuation
        total_score = (
        ...
        if total_score >= 7.5:  # High standards for quality
            signal = "bullish"
        ...
        analysis_data[ticker] = {
            ...
            "research_context": data.get("research_context", "No external documents provided.")
        }

        
        progress.update_status(agent_id, ticker, "Generating quality investing analysis")
        quality_output = generate_quality_output(
            ticker=ticker, 
        ...
        quality_analysis_payload[ticker] = cot_to_analyst_payload(quality_output)
 
        progress.update_status(agent_id, ticker, "Done", analysis=quality_output.conviction_reason)
    
    # Wrap results in a single message for the chain
    message = HumanMessage(
        content=json.dumps(quality_analysis_payload),
        name=agent_id
    )
    
    # Show reasoning if requested
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(quality_analysis_payload, "Quality Investing Lead")

    
    # Add signals to the overall state
    state["data"]["analyst_signals"][agent_id] = quality_analysis_payload
...
def analyze_moat_strength(metrics: list, financial_line_items: list) -> dict:
    """
    Analyze the business's competitive advantage:
    ...
    # 1. Return on Invested Capital (ROIC) analysis - key quality metric
    ...
    if roic_values:
        # Check if ROIC consistently above 15% (quality threshold)
    ...
    if gross_margins and len(gross_margins) >= 3:
        # Consistent or improving gross margins indicate pricing power
    ...
    # 3. Capital intensity - focus on low capex businesses
    ...
    # 4. Intangible assets - value R&D and intellectual property
...
def analyze_management_quality(financial_line_items: list, insider_trades: list) -> dict:
    ...
    # High-quality companies convert earnings to cash
    ...
    # 2. Debt management - focus on conservative leverage
    ...
    # 3. Cash management efficiency - value appropriate cash levels
    ...
    if cash_values and revenue_values and len(cash_values) > 0 and len(revenue_values) > 0:
        # Calculate cash to revenue ratio (Ideal 10-20% for most businesses)
    ...
    # 4. Insider activity - value alignment of interest
    ...
    # 5. Consistency in share count - prefer stable/decreasing shares
...
def analyze_predictability(financial_line_items: list) -> dict:
    """
    Assess the predictability of the business - prioritize businesses
    ...
    # 3. Margin consistency - value stable margins
...
def calculate_quality_valuation(financial_line_items: list, market_cap: float) -> dict:
    """
    Calculate intrinsic value using quality investing principles:
    ...
    # Get FCF values (A key "owner earnings" metric)
    fcf_values = [item.free_cash_flow for item in financial_line_items 
                 if hasattr(item, 'free_cash_flow') and item.free_cash_flow is not None]
    ...
    # (Normalize earnings to avoid over/under-valuation based on cyclical factors)
    ...
    # 3. Apply FCF multiple based on business quality
    # Pay higher multiples for wonderful businesses
    ...
    # Use straightforward valuations, avoiding complex DCF models
    ...
    # Focused on growing owner earnings
    if len(fcf_values) >= 3:
...
def analyze_news_sentiment(news_items: list) -> str:
    ...
    Pay attention to significant news but don't overreact to short-term stories.
    """
...
def make_quality_facts_bundle(analysis: dict[str, any]) -> dict[str, any]:
    ...
    return {
        ...
        "primary_research": analysis.get("research_context")
    }
...
def compute_confidence(analysis: dict, signal: str) -> int:
    ...
    # Quality dominates: 0.35*moat + 0.25*mgmt + 0.25*pred (max 8.5)
    ...
    # Ensure bucket semantics by clamping into quality buckets depending on signal
    if signal == "bullish":
...
def generate_quality_output(
    ...
    facts_bundle = make_quality_facts_bundle(analysis_data)
    template = ChatPromptTemplate.from_messages([
        ("system",
         """YOUR INVESTMENT PHILOSOPHY:
         1. Quality of Business: Moats and predictable cash flows.
         2. Quality of Management: Integrity and capital allocation.
         3. Compounding: Long-term focus.
         4. Standard of Accuracy: Prefer being vaguely right over precisely wrong.

         ADAPTIVE INTELLIGENCE RULES (Institutional Priority):
         1. Prioritize 'primary_research' over external API data.
         2. If 'primary_research' contradicts API metrics, favor the research (especially on management quality).
        ...
        ("human",
         "Ticker: {ticker}\n"
         "Facts:\n{facts}\n"
         "Prior confidence hint (0-100): {confidence}\n"
    ...
    return call_llm(
        ...
        agent_name=agent_id,
        state=state,
        default_factory=lambda: default_cot("quality_investing_lead"),
    )
