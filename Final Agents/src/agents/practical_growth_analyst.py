...
def practical_growth_analyst_agent(state: AgentState, agent_id: str = "practical_growth_analyst"):
    """
    Analyzes stocks using practical growth principles:
      - Invest in clear, understandable businesses.
      - Growth at a Reasonable Price (GARP), emphasizing the PEG ratio.
      - Look for consistent revenue & EPS increases and manageable debt.
      - Be alert for high-growth opportunities.
    ...
    analysis_data = {}
    growth_analysis_payload = {}

    for ticker in tickers:
        progress.update_status(agent_id, ticker, "Gathering financial line items")
        # Relevant line items for a practical growth approach
        ...
        growth_analysis = analyze_practical_growth(financial_line_items)

        progress.update_status(agent_id, ticker, "Analyzing fundamentals")
        fundamentals_analysis = analyze_practical_fundamentals(financial_line_items)

        progress.update_status(agent_id, ticker, "Analyzing valuation (focus on PEG)")
        valuation_analysis = analyze_practical_valuation(financial_line_items, market_cap)
        ...
        # Combine partial scores with typical weights:
        ...
        progress.update_status(agent_id, ticker, "Generating practical growth analysis")
        growth_output = generate_practical_growth_output(
            ticker=ticker,
        ...
        growth_analysis_payload[ticker] = cot_to_analyst_payload(growth_output)

        progress.update_status(agent_id, ticker, "Done", analysis=growth_output.conviction_reason)

    # Wrap up results
    message = HumanMessage(content=json.dumps(growth_analysis_payload), name=agent_id)

    if state["metadata"].get("show_reasoning"):
        show_agent_reasoning(growth_analysis_payload, "Practical Growth Analyst")

    # Save signals to state
    state["data"]["analyst_signals"][agent_id] = growth_analysis_payload


    return {"messages": [message], "data": state["data"]}


def analyze_practical_growth(financial_line_items: list) -> dict:
    """
    Evaluate growth based on revenue and EPS trends:
      - Consistent revenue growth
      - Consistent EPS growth
    Prioritize companies with steady, understandable growth.
    """
...
def analyze_practical_fundamentals(financial_line_items: list) -> dict:
    """
    Evaluate basic fundamentals:
      - Debt/Equity
      - Operating margin (or gross margin)
      - Positive Free Cash Flow
    Focus on avoidng heavily indebted or complicated businesses.
    """
...
def analyze_practical_valuation(financial_line_items: list, market_cap: float | None) -> dict:
    """
    Approach to 'Growth at a Reasonable Price' (GARP):
...
def generate_practical_growth_output(
    ticker: str,
    ...
    """
    Generates a final JSON signal in a practical and professional style.
    """
    template = ChatPromptTemplate.from_messages(
        [
            (
                ...
                2. Growth at a Reasonable Price (GARP): Use PEG ratio.
                3. High-Growth Potential: EPS expansion.
                4. Steady Growth: Consistent revenue.
                5. Low Debt: Avoid leverage.
            ...
            (
                "human",
                """Based on the following analysis data for {ticker}, produce a practical growth investment signal.
    ...
    return call_llm(
        ...
        agent_name=agent_id,
        state=state,
        default_factory=lambda: default_cot("practical_growth_analyst"),
    )
