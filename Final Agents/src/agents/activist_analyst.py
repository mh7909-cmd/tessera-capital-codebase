...
def activist_analyst_agent(state: AgentState, agent_id: str = "activist_analyst"):
    """
    Analyzes stocks using activist investing principles and LLM reasoning.
    ...
    for ticker in tickers:
        ...
        progress.update_status(agent_id, ticker, "Generating activist strategy analysis")
        activist_output = generate_activist_output(
            ticker=ticker, 
        ...
        activist_analysis_payload[ticker] = cot_to_analyst_payload(activist_output)

        progress.update_status(agent_id, ticker, "Done", analysis=activist_output.conviction_reason)
    
    # Wrap results in a single message for the chain
    message = HumanMessage(
        content=json.dumps(activist_analysis_payload),
        name=agent_id
    )
    
    # Show reasoning if requested
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(activist_analysis_payload, "Activist Strategy Lead")
    
    # Add signals to the overall state
    state["data"]["analyst_signals"][agent_id] = activist_analysis_payload
...
def analyze_activism_potential(financial_line_items: list) -> dict:
    """
    Activist investors often engage if a company has a decent brand or moat
    ...
    # Suppose if there's decent revenue growth but margins are below 10%, activism potential is high.
...
def analyze_valuation(financial_line_items: list, market_cap: float) -> dict:
    """
    Invest in companies trading at a discount to intrinsic value.
...
def generate_activist_output(
    ...
    """
    Generates investment decisions using activist investing principles.
    ...
    template = ChatPromptTemplate.from_messages([
        (
            "system",
            """You are an Activist Strategy Lead, making investment decisions:

                YOUR INVESTMENT PHILOSOPHY:
                1. Identify opportunities for value creation through operational or governance changes.
                2. Focus on capital allocation, board composition, and strategic direction.
                3. Prioritize businesses with significant 'hidden value' and clear catalysts.
        ...
        (
            "human",
            """Based on the following analysis, create an activist-style investment signal.
    ...
    return call_llm(
        ...
        agent_name=agent_id,
        state=state,
        default_factory=lambda: default_cot("activist_analyst"),
    )
