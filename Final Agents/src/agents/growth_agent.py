...
def growth_analyst_agent(state: AgentState, agent_id: str = "growth_analyst_agent"):
    ...
    for ticker in tickers:
        ...
        # Add research context and final evaluation
        reasoning = {}
        ...
        reasoning["final_analysis"] = {
            ...
            "institutional_alignment": "Favoring primary research findings over raw API metrics where applicable."
        }

        growth_analysis[ticker] = enrich_deterministic_cot(
            agent_key="growth_analyst",
            philosophy="ADAPTIVE INTELLIGENCE: Reward sustainable growth and margin expansion, synchronized with institutional research context and insider alignment.",
            criteria_bullets=[
                f"Growth score {scores['growth']}",
                f"Valuation score {scores['valuation']}",
                f"Margins {scores['margins']}",
                f"Insider {scores['insider']}",
                f"Health {scores['health']}",
            ],
            signal=signal,
            confidence=confidence,
            conviction_reason=f"Institutional growth-factor ensemble favors {signal} with {weighted_score:.2f} score.",
            invalidation_condition="Growth decelerates materially vs research expectations or balance sheet stress emerges.",
