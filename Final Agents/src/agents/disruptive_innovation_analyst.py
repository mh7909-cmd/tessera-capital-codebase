...
def disruptive_innovation_analyst_agent(state: AgentState, agent_id: str = "disruptive_innovation_analyst"):
    """
    Analyzes stocks using disruptive innovation principles and LLM reasoning:
    ...
    for ticker in tickers:
        ...
        valuation_analysis = analyze_innovation_valuation(financial_line_items, market_cap)
        ...
        progress.update_status(agent_id, ticker, "Generating disruptive innovation analysis")
        innovation_output = generate_innovation_output(
            ticker=ticker,
        ...
        innovation_analysis_payload[ticker] = cot_to_analyst_payload(innovation_output)

        progress.update_status(agent_id, ticker, "Done", analysis=innovation_output.conviction_reason)

    message = HumanMessage(content=json.dumps(innovation_analysis_payload), name=agent_id)

    if state["metadata"].get("show_reasoning"):
        show_agent_reasoning(innovation_analysis_payload, "Disruptive Innovation Analyst")

    state["data"]["analyst_signals"][agent_id] = innovation_analysis_payload
...
def analyze_innovation_valuation(financial_line_items: list, market_cap: float) -> dict:
    """
    Focused on long-term exponential growth potential. We can do
...
def generate_innovation_output(
    ticker: str,
    ...
    """
    Generates investment decisions based on disruptive innovation.
    """
    template = ChatPromptTemplate.from_messages(
        [
            (
                ...
                1. Focus on companies introducing disruptive technologies or business models.
                2. Prioritize massive total addressable markets (TAM) and high-growth potential.
                3. Reward innovation, visionary leadership, and scalability over short-term earnings.
            ...
            (
                "human",
                """Based on the following analysis, create a disruptive-innovation investment signal.
    ...
    return call_llm(
        ...
        agent_name=agent_id,
        state=state,
        default_factory=lambda: default_cot("disruptive_innovation_analyst"),
    )
