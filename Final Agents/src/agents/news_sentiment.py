...
# Configuration for fast sentiment analysis
...
def news_sentiment_agent(state: AgentState, agent_id: str = "news_sentiment_agent"):
    ...
    for ticker in tickers:
        ...
        # Add news sentiment analysis metrics
        reasoning = {}
        reasoning["news_sentiment"] = {
        ...
        sentiment_analysis[ticker] = enrich_deterministic_cot(
            agent_key="news_sentiment_analyst",
            philosophy="ADAPTIVE INTELLIGENCE: Infer institutional positioning from headline sentiment and narrative tone, cross-referenced with primary research.",
            criteria_bullets=[
                f"Bull articles {bullish_signals}",
                f"Bear articles {bearish_signals}",
                f"Neutral {neutral_signals}",
                f"LLM-classified count {sentiments_classified_by_llm}",
            ],
            signal=overall_signal,
            confidence=confidence,
            conviction_reason=f"Institutional news flow ensemble favors {overall_signal} with {confidence:.0f}% confidence.",
            invalidation_condition="Narrative shift: news flow diverges from primary research findings.",
