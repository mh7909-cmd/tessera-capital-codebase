...
def emerging_market_analyst_agent(state: AgentState, agent_id: str = "emerging_market_analyst"):
    """Analyzes stocks using emerging markets growth principles and LLM reasoning."""
    data = state["data"]
    ...
    analysis_data = {}
    emerging_market_analysis_payload = {}
    ...
    for ticker in tickers:
        ...
        # Decision rules (30% minimum margin of safety for conviction)
        ...
        intrinsic_value_analysis = analyze_emerging_market_style(
        ...
        # ─── LLM: craft emerging market narrative ──────────────────────────────
        progress.update_status(agent_id, ticker, "Generating emerging market analysis")
        emerging_market_output = generate_emerging_market_output(
            ticker=ticker,
        ...
        emerging_market_analysis_payload[ticker] = cot_to_analyst_payload(emerging_market_output)
 
        progress.update_status(agent_id, ticker, "Done", analysis=emerging_market_output.conviction_reason)

    # ─── Push message back to graph state ──────────────────────────────────────
    message = HumanMessage(content=json.dumps(emerging_market_analysis_payload), name=agent_id)
 
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(emerging_market_analysis_payload, "Emerging Markets Analyst")
 
    state["data"]["analyst_signals"][agent_id] = emerging_market_analysis_payload
...
def analyze_growth(financial_line_items: list) -> dict[str, any]:
    ...
    Focus on companies with strong, consistent compound growth.
    """
...
def analyze_balance_sheet(financial_line_items: list) -> dict[str, any]:
    ...
    Focus on companies with clean balance sheets and manageable debt.
    """
    if not financial_line_items:
...
def analyze_cash_flow(financial_line_items: list) -> dict[str, any]:
    ...
    Prioritize companies generating strong free cash flow and rewarding shareholders.
    """
    if not financial_line_items:
...
def analyze_management_actions(financial_line_items: list) -> dict[str, any]:
    ...
    Focus on managements who buy back shares or avoid dilution.
    """
    if not financial_line_items:
...
def assess_quality_metrics(financial_line_items: list) -> float:
    """
    Assess company quality based on institutional growth criteria.
...
def calculate_intrinsic_value(financial_line_items: list, market_cap: float) -> float:
    """
    Calculate intrinsic value using emerging market growth principles:
...
def analyze_emerging_market_style(
    financial_line_items: list,
    owner_earnings: float = None,
    intrinsic_value: float = None,
    current_price: float = None,
    research_context: str = "No external documents provided.",
) -> dict[str, any]:
    """
    Comprehensive analysis in an institutional emerging markets style.
    """
    # Run sub-analyses
...
# LLM generation
# ────────────────────────────────────────────────────────────────────────────────
def generate_emerging_market_output(
    ticker: str,
    analysis_data: dict[str, any],
    state: AgentState,
    agent_id: str,
) -> CotAgentStructuredOutput:
    """Get investment decision from LLM with institutional emerging market principles"""
    template = ChatPromptTemplate.from_messages(
        [
            ...
            (
                "human",
                """Based on the following data, create a professional emerging markets investment signal:
    ...
    return call_llm(
        ...
        state=state,
        agent_name=agent_id,
        default_factory=lambda: default_cot("emerging_market_analyst"),
    )