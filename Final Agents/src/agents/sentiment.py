...
def sentiment_analyst_agent(state: AgentState, agent_id: str = "sentiment_analyst_agent"):
    ...
    for ticker in tickers:
        ...
        # Add research context and combined analysis
        ...
        reasoning["combined_analysis"] = {
            ...
            "signal_determination": f"{'Bullish' if bullish_signals > bearish_signals else 'Bearish' if bearish_signals > bullish_signals else 'Neutral'} based on weighted institutional signal comparison"
        }

        sentiment_analysis[ticker] = enrich_deterministic_cot(
            agent_key="sentiment_analyst",
            philosophy="ADAPTIVE INTELLIGENCE: Blend insider flow with narrative tone, prioritizing institutional research context over raw news sentiment.",
            criteria_bullets=[
                f"Insider tilt: {reasoning['insider_trading']['signal']}",
                f"News tilt: {reasoning['news_sentiment']['signal']}",
                f"Weighted bull {bullish_signals:.1f} vs bear {bearish_signals:.1f}",
            ],
            signal=overall_signal,
            confidence=confidence,
            conviction_reason=f"Institutional insider+news flow ensemble implies {overall_signal} with {confidence:.0f}% confidence.",
            invalidation_condition="Insider patterns shift or institutional research context contradicts headline sentiment.",
