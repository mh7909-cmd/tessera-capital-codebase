...
def risk_management_agent(state: AgentState, agent_id: str = "risk_management_agent"):
    ...
    for ticker in all_tickers:
        ...
        if not prices:
            # Fallback: if today's data is missing (e.g. weekend/early morning), try a broader range to get the latest close
            progress.update_status(agent_id, ticker, "No price data in range. Retrying with broader lookback...")
            prices = get_prices(
                ticker=ticker,
                start_date=data["start_date"],
        ...
        if not prices:
            progress.update_status(agent_id, ticker, "Warning: No price data found even with fallback")
    ...
    for ticker in tickers:
        ...
        # Audit log for troubleshooting 0-quantity trades
        audit_tag = f"[AUDIT: {ticker}] Val: ${total_portfolio_value:,.0f} | Cash: ${available_cash:,.0f} | PosLimit: ${position_limit:,.0f} | Available: ${max_position_size:,.0f}"
        progress.update_status(agent_id, ticker, audit_tag)
    ...
    # Add the signal to the analyst_signals delta
    # We return ONLY the updates to the 'data' key to ensure safe state merging
    return {
        "messages": state["messages"] + [message],
        "data": {
            "analyst_signals": {agent_id: risk_analysis},
