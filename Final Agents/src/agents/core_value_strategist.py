...
def core_value_strategist_agent(state: AgentState, agent_id: str = "core_value_strategist"):
    """Analyzes stocks using core value principles and LLM reasoning."""
    data = state["data"]
    ...
    for ticker in tickers:
        ...
        progress.update_status(agent_id, ticker, "Generating core value analysis")
        core_value_output = generate_core_value_output(
            ticker=ticker,
        ...
        core_value_analysis_payload[ticker] = cot_to_analyst_payload(core_value_output)
 
        progress.update_status(agent_id, ticker, "Done", analysis=core_value_output.conviction_reason)

    # Create the message
    message = HumanMessage(content=json.dumps(core_value_analysis_payload), name=agent_id)
 
    # Show reasoning if requested
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(core_value_analysis_payload, agent_id)
 
    # Add the signal to the analyst_signals list
    state["data"]["analyst_signals"][agent_id] = core_value_analysis_payload
...
def analyze_fundamentals(metrics: list) -> dict[str, any]:
    """Analyze company fundamentals based on core value criteria."""
    if not metrics:
...
def analyze_moat(metrics: list) -> dict[str, any]:
    ...
    Includes multiple moat indicators:
    ...
    # 1. Return on Capital Consistency (A key moat indicator)
...
def calculate_owner_earnings(financial_line_items: list) -> dict[str, any]:
    """
    Calculate owner earnings (a measure of true earnings power).
    Methodology: Net Income + Depreciation/Amortization - Maintenance CapEx - Working Capital Changes
...
def estimate_maintenance_capex(financial_line_items: list) -> float:
    ...
    Crucial for understanding true owner earnings.
    """
    if not financial_line_items:
        return 0
...
def calculate_intrinsic_value(financial_line_items: list) -> dict[str, any]:
    ...
    Uses sophisticated assumptions and a conservative approach.
    """
    ...
    # Apply additional margin of safety
...
def analyze_book_value_growth(financial_line_items: list) -> dict[str, any]:
    """Analyze book value per share growth - a key quality metric."""
...
def analyze_pricing_power(financial_line_items: list, metrics: list) -> dict[str, any]:
    """
    Analyze pricing power - a key indicator of a business moat.
...
def generate_core_value_output(
        ticker: str,
        analysis_data: dict[str, any],
        state: AgentState,
        agent_id: str = "core_value_strategist",
) -> CotAgentStructuredOutput:
    """Get investment decision from LLM based on core value principles."""
    ...
    facts = {
        ...
        "primary_research": analysis_data.get("research_context")
    }
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                ...
                1. Margin of Safety: Price vs Intrinsic Value.
                2. Owner Earnings: Real cash flow available to owners.
                3. Durable Moat: Competitive advantages that last.
                4. Management: Capital allocation excellence.
    ...
    def create_default_core_value_signal():
        return CotAgentStructuredOutput(
            agent="core_value_strategist",
            reasoning_steps=[
                "Value investing with a focus on durable moats and owner earnings.",
                "Insufficient data to apply criteria.",
                "Score 0 due to missing inputs.",
                "No conviction.",
                "Would change view if fundamentals became clear.",
            ],
            score=0,
    ...
    return call_llm(
        ...
        agent_name=agent_id,
        state=state,
        default_factory=create_default_core_value_signal,
    )
