...
def low_risk_value_analyst_agent(state: AgentState, agent_id: str = "low_risk_value_analyst"):
    """Evaluate stocks using a checklist-based 'heads I win, tails I don't lose much' approach."""
    data = state["data"]
    ...
    low_risk_analysis_payload: dict[str, any] = {}

    # Focus on: downside protection, simple business, moat via unit economics, FCF yield vs alternatives,
    ...
    for ticker in tickers:
        ...
        valuation = analyze_low_risk_valuation(line_items, market_cap)
        ...
        # Combine to an overall score: heavily weight downside and cash yield
        total_score = (
            downside["score"] * 0.45
        ...
        progress.update_status(agent_id, ticker, "Generating low-risk strategy analysis")
        low_risk_output = generate_low_risk_output(
            ticker=ticker,
        ...
        low_risk_analysis_payload[ticker] = cot_to_analyst_payload(low_risk_output)

        progress.update_status(agent_id, ticker, "Done", analysis=low_risk_output.conviction_reason)

    message = HumanMessage(content=json.dumps(low_risk_analysis_payload), name=agent_id)

    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(low_risk_analysis_payload, "Low-Risk Value Analyst")


    state["data"]["analyst_signals"][agent_id] = low_risk_analysis_payload
...
def analyze_low_risk_valuation(financial_line_items: list, market_cap: float | None) -> dict[str, any]:
...
def analyze_double_potential(financial_line_items: list, market_cap: float | None) -> dict[str, any]:
    ...
    tmp_val = analyze_low_risk_valuation(financial_line_items, market_cap)
...
def generate_low_risk_output(
    ticker: str,
    analysis_data: dict[str, any],
    state: AgentState,
    agent_id: str,
) -> CotAgentStructuredOutput:
    """Generate low-risk, high-uncertainty focused trading signals."""
    template = ChatPromptTemplate.from_messages([
        (
            "system",
            """YOUR INVESTMENT PHILOSOPHY:
            1. Heads, I win; tails, I don't lose much.
            2. Prioritize capital preservation and low-volatility value.
            3. Seek stable, cash-generative businesses with fortress balance sheets.
    ...
    return call_llm(
        ...
        agent_name=agent_id,
        default_factory=lambda: default_cot("low_risk_value_analyst"),
    )