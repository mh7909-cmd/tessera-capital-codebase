...
def global_macro_lead_agent(state: AgentState, agent_id: str = "global_macro_lead"):
    """
    Analyzes stocks using global macro investing principles:
    ...
    for ticker in tickers:
        ...
        # Include relevant line items for a global macro approach:
        ...
        progress.update_status(agent_id, ticker, "Performing global macro valuation")
        valuation_analysis = analyze_global_macro_valuation(financial_line_items, market_cap)
        ...
        progress.update_status(agent_id, ticker, "Generating global macro analysis")
        macro_output = generate_global_macro_output(
            ticker=ticker,
        ...
        macro_analysis_payload[ticker] = cot_to_analyst_payload(macro_output)

        progress.update_status(agent_id, ticker, "Done", analysis=macro_output.conviction_reason)

    # Wrap results in a single message
    message = HumanMessage(content=json.dumps(macro_analysis_payload), name=agent_id)

    if state["metadata"].get("show_reasoning"):
        show_agent_reasoning(macro_analysis_payload, "Global Macro Strategist")

    state["data"]["analyst_signals"][agent_id] = macro_analysis_payload
...
def analyze_global_macro_valuation(financial_line_items: list, market_cap: float | None) -> dict:
    """
    Macro approach to valuation:
      - P/E
      - P/FCF
      - EV/EBIT
      - EV/EBITDA
...
def generate_global_macro_output(
    ticker: str,
    ...
    """
    Generates a JSON signal in an institutional global macro style.
    """
    template = ChatPromptTemplate.from_messages(
        [
            (
              ...
              1. Reflexivity: Prices influence fundamentals.
              2. Top-Down: Macro trends drive asset classes.
              3. Risk/Reward: Focus on asymmetric setups.
              4. Global Scope: Currencies, rates, and commodities.
            ...
            (
              "human",
              """Based on the following analysis, create a global macro investment signal.
    ...
    return call_llm(
        ...
        agent_name=agent_id,
        state=state,
        default_factory=lambda: default_cot("global_macro_lead"),
    )
