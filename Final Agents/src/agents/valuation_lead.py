...
def valuation_lead_agent(state: AgentState, agent_id: str = "valuation_lead"):
    """
    Analyze US equities through an intrinsic-valuation lens:
    ...
    Produces a trading signal and explanation in a disciplined analytical voice.
    """
    data      = state["data"]
    ...
    for ticker in tickers:
        ...
        # Decision rules (Institutional standards typically require ~25% Margin of Safety)
        ...
        analysis_data[ticker] = {
            ...
            "relative_valuation": relative_val_analysis,
        ...
        valuation_signals[ticker] = cot_to_analyst_payload(valuation_output)

        progress.update_status(agent_id, ticker, "Done", analysis=valuation_output.conviction_reason)

    # ─── Push message back to graph state ──────────────────────────────────────
    message = HumanMessage(content=json.dumps(valuation_signals), name=agent_id)

    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(valuation_signals, "Intrinsic Valuation Lead")

    state["data"]["analyst_signals"][agent_id] = valuation_signals
...
def estimate_cost_of_equity(beta: float | None) -> float:
    """CAPM: r_e = r_f + β × ERP (use historical institutional averages)."""
...
def generate_valuation_output(
    ...
    """
    Ask the LLM to channel an institutional valuation style:
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                ...
                - Valuation: Story → Numbers → Value narrative.
                - Discipline: Reinvestment and risk-adjusted growth.
                - MOS: Margin of Safety based on intrinsic value.
    ...
    return call_llm(
        ...
        agent_name=agent_id,
        state=state,
        default_factory=lambda: default_cot("valuation_lead"),
    )
