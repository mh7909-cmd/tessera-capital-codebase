...
class AgentProgress:
    ...
    def start(self):
        ...
        # Institutional Silence for piped logs: 
        # We skip live table updates to avoid log-spam in the webhook server.
        self.started = True
    ...
    def update_status(self, agent_name: str, ticker: Optional[str] = None, status: str = "", analysis: Optional[str] = None):
        ...
        # Institutional Silence: Simple sequential logging
        # Robust filter for ghost logs with empty or None tickers
        if not ticker or str(ticker).strip().upper() in ["NONE", ""]:
            return
        ...
        if status.lower() == "done":
            time.sleep(0.2)
            print(f"   ✅ {self._get_display_name(agent_name)}{ticker_display} analysis complete.")
        elif status.lower() == "error":
            time.sleep(0.2)
            print(f"   ❌ {self._get_display_name(agent_name)}{ticker_display} encountered an error.")
        elif status:
            time.sleep(0.1)
            print(f"   🔄 {self._get_display_name(agent_name)}{ticker_display}: {status}...")
